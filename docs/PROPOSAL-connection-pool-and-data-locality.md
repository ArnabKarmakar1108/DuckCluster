...
- [ ] **A3. Dedicated admin connection** *(deferred to Phase C — not needed until ShardFileWatcher)*
...
### Phase E: Remote Read Fallback (Coordinator-Mediated)

- [ ] **E1. Add** `ReadShard` **RPC to** `WorkerService`
  - New streaming RPC: `rpc ReadShard(ReadShardRequest) returns (stream RowBatch)`
  - `ReadShardRequest { string table_name; uint32 shard_id; optional string filter_sql; }`
  - Worker reads from its local ATTACHed shard (`SELECT * FROM <table>_shard<id>.<table> [WHERE ...]`) and streams rows
  - Uses the connection pool (same as ExecuteFragment)

- [ ] **E2. Coordinator remote-read path**
  - Triggered when: all owners of a shard are either DOWN or returned `RESOURCE_EXHAUSTED`
  - Flow:
    1. Coordinator picks the least-loaded non-owner worker as the execution target
    2. Coordinator calls `ReadShard` on one of the shard owners (pick healthiest) to stream the data
    3. Coordinator pushes streamed data into a temp table on the execution target (via a new `LoadTempData` RPC or by embedding data in the `FragmentRequest`)
    4. Coordinator dispatches the fragment to the execution target with SQL referencing the temp table
    5. Temp table is dropped after fragment completes
  - This keeps the v1 communication pattern simple: coordinator ↔ workers only, no worker-to-worker calls

- [ ] **E3. Integration test: remote read fallback**
  - Start 3 workers, each with 1 shard (replication factor = 1, to force remote read)
  - Exhaust worker-1's pool (fill with long-running `SELECT pg_sleep(...)` equivalent)
  - Send a query for worker-1's shard
  - Verify coordinator streams from worker-1 via `ReadShard`, pushes to worker-2, executes there, returns correct results
